...
export function ProviderOrderTrackingMap({ orderId }: ProviderOrderTrackingMapProps) {
  const mapContainer = useRef<HTMLDivElement>(null);
  const map = useRef<mapboxgl.Map | null>(null);
  const driverMarker = useRef<mapboxgl.Marker | null>(null);
  
  const { data: mapboxToken, isLoading: isLoadingToken } = useMapboxToken();
  const { order, tracking, routeHistory, isLoading, isOutForDelivery, storeLocation } = useProviderOrderTracking(orderId);
  ...
  // Initialize map
  useEffect(() => {
    if (!mapContainer.current || !mapboxToken || !storeLocation || !order?.delivery_lat || !order?.delivery_lng) return;

    mapboxgl.accessToken = mapboxToken;

    const deliveryLocation = { lat: order.delivery_lat, lng: order.delivery_lng };
    ...
    new mapboxgl.Marker({ element: storeEl })
      ...
      .setPopup(new mapboxgl.Popup({ offset: 25, closeButton: false })
        .setHTML(`<div class="font-arabic p-2 text-center font-bold text-green-600">🏪 ${order.service_providers?.business_name || 'المتجر'}</div>`))
      .addTo(map.current);
    ...
    new mapboxgl.Marker({ color: '#ea580c' })
      ...
      .setPopup(new mapboxgl.Popup({ offset: 25, closeButton: false })
        .setHTML(`<div class="font-arabic p-2 text-center font-bold text-orange-600">📍 موقع التوصيل</div>`))
      .addTo(map.current);
    ...
    map.current.on('load', () => {
      ...
      // Add expected route line - dashed
  ...
  if (!mapboxToken || !storeLocation || !order?.delivery_lat || !order?.delivery_lng) {
    return (
      <Card>
        <CardContent className="p-4 text-center">
          <p className="text-muted-foreground text-sm">لا يمكن عرض الخريطة</p>
        </CardContent>
      </Card>
    );
  }
